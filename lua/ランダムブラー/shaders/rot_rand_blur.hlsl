...
float4 rot_rand_blur(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_TARGET {
  ...
  float theta =
      (0.5 - random_value_at((uint2)pixel, (uint)constants.seed, randomWidth, randomHeight) -
       constants.basePosition * 0.5) *
      (constants.maxOffset * PI / 180.0);
  float sin_theta = sin(theta);
  float cos_theta = cos(theta);
  int2 sample_pixel = int2(round(float2(
      origin.x + offset.x * cos_theta + offset.y * sin_theta,
      origin.y + offset.y * cos_theta - offset.x * sin_theta)));
  sample_pixel = clamp(sample_pixel, int2(0, 0), int2((int)width - 1, (int)height - 1));
