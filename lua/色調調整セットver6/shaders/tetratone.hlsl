...
float4 tetratone(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_TARGET {
  ...
  float3 col4 = float3(constants.color4R, constants.color4G, constants.color4B);

  float3 color = lerp(
      lerp(col1, col2,
           unlerpClamped(metric, constants.threshold1, constants.threshold2)),
      lerp(col3, col4,
           unlerpClamped(metric, constants.threshold3, constants.threshold4)),
      unlerpClamped(metric, constants.threshold2, constants.threshold3));
