...
float4 monochromatic2(float4 pos : SV_Position, float2 uv : TEXCOORD0)
  ...
  float luma =
      (rgb255.b * 0.456 + rgb255.g * 2.348 + rgb255.r * 1.196) / 1023.0;
  float idx =
      clamp(floor(pow(max(luma, 0.0), 1.0 / gamma) * 1023.0), 0.0, 1023.0);
  float base = idx * 0.25;

  float outR = clamp(constants.u * 357.663 + base, 0.0, 255.0);
  float outG =
      clamp(base - constants.v * 87.822 - constants.u * 181.407, 0.0, 255.0);
