...
void accumulate_sample(inout BilinearContribution contribution, float4 sample,
                       float weight) {
...
BilinearContribution sample_bilinear_legacy(float2 samplePixel, uint width,
                                            uint height) {
  ...
  accumulate_sample(contribution, srcTex.Load(int3(x0, y0, 0)),
                    (1.0 - frac.x) * (1.0 - frac.y));
  accumulate_sample(contribution, srcTex.Load(int3(x1, y0, 0)),
                    frac.x * (1.0 - frac.y));
  accumulate_sample(contribution, srcTex.Load(int3(x1, y1, 0)),
                    frac.x * frac.y);
  accumulate_sample(contribution, srcTex.Load(int3(x0, y1, 0)),
                    (1.0 - frac.x) * frac.y);
  return contribution;
}

float2 rotate_offset(float2 origin, float2 offset, float sinTheta,
                     float cosTheta) {
  return float2(origin.x + offset.x * cosTheta + offset.y * sinTheta,
...
float4 rot_blur(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_TARGET {
  ...
  float2 posSamplePixel =
      rotate_offset(origin, offset, constants.sinPos, constants.cosPos);
  float2 negSamplePixel =
      rotate_offset(origin, offset, constants.sinNeg, constants.cosNeg);

  if (constants.highQuality > 0.5) {
    BilinearContribution posSample =
        sample_bilinear_legacy(posSamplePixel, width, height);
    BilinearContribution negSample =
        sample_bilinear_legacy(negSamplePixel, width, height);
    float alphaSum = posSample.alpha + negSample.alpha;
    if (alphaSum <= 0.0) {
      return float4((posSample.raw + negSample.raw) * 0.5, 0.0);
    }
    return float4((posSample.premul + negSample.premul) / alphaSum,
                  alphaSum * 0.5);
  }
  ...
  return float4((posSample.rgb * alphaPos + negSample.rgb * alphaNeg) /
                    alphaSum,
