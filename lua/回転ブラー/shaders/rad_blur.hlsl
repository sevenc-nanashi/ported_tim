...
float4 rad_blur(float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_TARGET {
  ...
  float2 outerSamplePixel =
      float2(offset.x * constants.outerScale + origin.x,
  ...
  if (weightedAlpha > 0.0) {
    return float4(
        (innerPixel.rgb * innerPixel.a * constants.innerScale +
  ...
  return float4(
      (innerPixel.rgb * constants.innerScale +
